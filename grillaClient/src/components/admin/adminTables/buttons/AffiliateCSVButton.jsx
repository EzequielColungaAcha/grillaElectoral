...
export const AffiliateCSVButton = ({
  datos,
  setAffiliateList,
  setRefererList,
}) => {
      ...
      Papa.parse(file, {
        ...
        complete: function (results) {
          results.data.map((row) => {
            const dni = row.dni.replaceAll('.', '');
            dniArray.push(dni);
            if (row.referente) {
              refererArray.push({ dni: dni, referer: row.referente });
            }
          });
          setAffiliateList(dniArray);
          setRefererList(refererArray);
    ...
    <>
      ...
      >
        <div className='space-y-3 text-sm'>
          <p>
            El archivo CSV debe contener solo la cabecera <strong>dni</strong>
          </p>
